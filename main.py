...
def register_api(request: Request):
    ...
    message = ''
    return templates.TemplateResponse(request=request, name="admin_register.html", context={"error": message})
    ...
    if email_checker(email):
        ...
        if user:
            ...
            if password != confirm_password:
                ...
                return templates.TemplateResponse(request=request, name="admin_login.html", context={"success": message})
...
def login_api(request: Request):
    is_token = request.cookies.get('token')
    if not is_token:
        return templates.TemplateResponse(request=request, name="tour_login.html")
...
def login_api(request: Request, user_name: str = Form(...), email: str = Form(...), password: str = Form(...),
    ...
    if is_agent and is_agent.user_status:
        if email_checker(email):
            ...
            error = "Invalid credentials!"
    return templates.TemplateResponse(request=request, name="tour_login.html", context={"error": error})
...
def home_api(request: Request, db: Session = Depends(get_db)):
    ...
    if is_agent:
        ...
        return templates.TemplateResponse(request=request, name="base.html",
                                          context={
                                              "admin_data": is_agent, "is_agent": True,
                                              "current_date": datetime.date.today(),
                                              "home_images": home_images,
                                              "all_booking_data": all_booking_data})
...
def users_api(request: Request, db: Session = Depends(get_db), search: str = None):
    ...
    if is_admin:
        ...
        return templates.TemplateResponse(request=request, name="users.html",
                                          context={"admin_data": is_admin, "all_users": all_users})

    is_agent = get_one_db_data(db, tour_models.Agents, tour_models.Agents.user_token, is_token)
    if is_agent:
        return templates.TemplateResponse(request=request, name="users.html",
                                          context={"admin_data": is_agent, "is_agent": True})
...
def add_user_api(request: Request, db: Session = Depends(get_db)):
    ...
    if is_admin:
        return templates.TemplateResponse(request=request, name="add_user.html",
                                          context={"admin_data": is_admin})
...
def update_user_api(request: Request, data_id: int, db: Session = Depends(get_db)):
    ...
    if is_admin:
        user_data = get_one_db_data(db, tour_models.Agents, tour_models.Agents.id, data_id)
        return templates.TemplateResponse(request=request, name="add_user.html",
                                          context={"admin_data": is_admin, "user_data": user_data})
...
def update_user_api(request: Request, data_id: int, agency_name: str = Form(...), address: str = Form(...),
    ...
    if not user_data:
        return templates.TemplateResponse(request=request, name="add_user.html",
                                          context={"admin_data": is_admin, "error": "User not found."})
    ...
    try:
        ...
        return templates.TemplateResponse(request=request, name="add_user.html",
                                          context={
                                              "admin_data": is_admin, "user_data": user_data,
...
def update_profile_api(request: Request, db: Session = Depends(get_db)):
    ...
    is_admin = get_one_db_data(db, tour_models.Admin, tour_models.Admin.user_token, is_token)
    if is_admin:
        return templates.TemplateResponse(request=request, name="update_profile.html",
                                          context={"admin_data": is_admin})
...
def update_profile_api(request: Request, name: str = Form(...), email: str = Form(...), password: str = Form(...),
    ...
    if is_admin:
        try:
            ...
            return templates.TemplateResponse(request=request, name="update_profile.html",
                                              context={
                                                  "admin_data": is_admin, "is_agent": True,
                                                  "success": "Profile updated successfully."})
        except Exception as e:
            db.rollback()
            return templates.TemplateResponse(request=request, name="update_profile.html",
                                              context={"admin_data": is_admin, "is_agent": True,
                                                      "error": f"Something went wrong: {str(e)}"})
...
def booking_api(request: Request, db: Session = Depends(get_db)):
    ...
    if is_admin:
        ...
        return templates.TemplateResponse(request=request, name="booking.html",
                                          context={
                                              "admin_data": is_admin, "current_date": datetime.date.today(),
                                              "all_booking_data": all_booking_data})
...
def add_booking_api(request: Request, db: Session = Depends(get_db)):
    ...
    if is_admin:
        ...
        return templates.TemplateResponse(request=request, name="add_booking.html",
                                          context={"admin_data": is_admin, "agents_data": agents_data})
...
def booking_detail_api(request: Request, data_id: int, db: Session = Depends(get_db)):
    ...
    if is_admin:
        ...
        return templates.TemplateResponse(request=request, name="booking_detail.html",
                                          context={"admin_data": is_admin, "data_id": data_id,
...
def edit_booking_api(request: Request, data_id: int, db: Session = Depends(get_db)):
    ...
    if is_admin:
        ...
        return templates.TemplateResponse(request=request, name="edit_booking.html",
                                          context={"admin_data": is_admin, "agents_data": agents_data,
                                                   "data_id": data_id, "booking_data": booking_data})
...
def add_booking_days_api(request: Request, data_id: int, db: Session = Depends(get_db)):
    ...
    if is_admin:
        booking_data = get_one_db_data(db, tour_models.Bookings, tour_models.Bookings.id, data_id)
        return templates.TemplateResponse(request=request, name="add_booking_days.html",
                                          context={"admin_data": is_admin, "booking_data": booking_data,
                                                   "data_id": data_id})
    ...
    if is_agent:
        booking_data = get_one_db_data(db, tour_models.Bookings, tour_models.Bookings.id, data_id)
        return templates.TemplateResponse(request=request, name="add_booking_days.html",
                                          context={"admin_data": is_agent, "booking_data": booking_data,
...
async def add_booking_days_api(request: Request, data_id: int, booking_title: str = Form(...),
    ...
    if is_admin:
        try:
            ...
            return templates.TemplateResponse(request=request, name="add_booking_days.html",
                                              context={"admin_data": is_admin, "booking_data": booking_data,
                                                       "data_id": data_id, "success": "Booking added successfully!"})
            ...
            return templates.TemplateResponse(request=request, name="add_booking_days.html",
                                              context={"admin_data": is_admin, "booking_data": booking_data,
                                                       "data_id": data_id, "error": f"Error adding booking: {str(e)}"})
...
def edit_booking_day_api(request: Request, data_id: int, day_id: int, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request=request, name="edit_booking_days.html",
                                      context={"admin_data": is_admin, "data_id": data_id,
...
def booking_status_api(request: Request, db: Session = Depends(get_db)):
    ...
    if is_admin:
        ...
        return templates.TemplateResponse(request=request, name="booking_status.html",
                                          context={"admin_data": is_admin, "edit_booking_status": edit_booking_status,
                                                   "is_agent": False, "booking_map": booking_map,
    ...
    if is_agent:
        ...
        return templates.TemplateResponse(request=request, name="booking_status.html",
                                          context={"admin_data": is_agent, "edit_booking_status": edit_booking_status,
...
def booking_status_api(request: Request, db: Session = Depends(get_db)):
    ...
    if is_admin:
        ...
        return templates.TemplateResponse(request=request, name="booking_status.html",
                                          context={"admin_data": is_admin,
                                                   ...
                                                   "is_agent": False, "booking_map": {booking.id: booking},
                                                   "booking_event_map": booking_event_map})
    ...
    if is_agent:
        ...
        return templates.TemplateResponse(request=request, name="booking_status.html",
                                          context={"admin_data": is_agent,
                                                   "edit_booking_status": edit_booking_status,
                                                   "is_agent": True, "booking_map": {booking.id: booking},
                                                   "booking_event_map": booking_event_map})
...
def booking_day_status_api(request: Request, data_id: int, day_id: int, db: Session = Depends(get_db)):
    ...
    if is_admin:
        ...
        return templates.TemplateResponse(request=request, name="booking_status.html",
                                          context={"admin_data": is_admin, "day_id": day_id,
                                                   "is_agent": False, "data_id": data_id,
    ...
    if is_agent:
        ...
        return templates.TemplateResponse(request=request, name="booking_status.html",
                                          context={"admin_data": is_agent, "day_id": day_id,
                                                   "is_agent": True, "data_id": data_id,
                                                   "booking_map": {booking.id: booking},
                                                   "booking_event_map": booking_event_map})
...
def add_booking_status_api(request: Request, data_id: int, day_id: int, db: Session = Depends(get_db)):
    ...
    if is_admin:
        agents_data = get_all_db_data(db, tour_models.Agents)
        return templates.TemplateResponse(request=request, name="add_booking_status.html",
                                          context={"admin_data": is_admin, "agents_data": agents_data,
    ...
    if is_agent:
        agents_data = get_all_db_data(db, tour_models.Agents)
        return templates.TemplateResponse(request=request, name="add_booking_status.html",
                                          context={"admin_data": is_agent, "data_id": data_id,
                                                   "day_id": day_id, "agents_data": agents_data,
                                                   "is_agent": True})
...
async def add_booking_status_api(request: Request, data_id: int, day_id: int, agent_name: str = Form(...),
    ...
    if is_admin:
        ...
        try:
            ...
            return templates.TemplateResponse(request=request, name="add_booking_status.html",
                                              context={"admin_data": is_admin, "success": message,
                                                       "data_id": data_id, "day_id": day_id,})
        except Exception as e:
            message = 'Something went wrong'
            return templates.TemplateResponse(request=request, name="add_booking_status.html",
                                              context={"admin_data": is_admin, "error": message,
                                                       "data_id": data_id, "day_id": day_id,})
    ...
    if is_agent:
        ...
        try:
            ...
            message = 'Status Added Successfully'
            return templates.TemplateResponse(request=request, name="add_booking_status.html",
                                              context={"admin_data": is_agent, "success": message,
                                                       "data_id": data_id, "day_id": day_id,
                                                       "is_agent": True})
        except Exception as e:
            message = 'Something went wrong'
            return templates.TemplateResponse(request=request, name="add_booking_status.html",
                                              context={"admin_data": is_agent, "error": message,
                                                       "data_id": data_id, "day_id": day_id,
                                                       "is_agent": True})
...
def add_home_images(request: Request, db: Session = Depends(get_db)):
    ...
    is_admin = get_one_db_data(db, tour_models.Admin, tour_models.Admin.user_token, is_token)
    if is_admin:
        return templates.TemplateResponse(request=request, name="add_home_images.html",
                                          context={"admin_data": is_admin})
